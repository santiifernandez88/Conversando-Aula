...
@Component({
  ...
  templateUrl: './chat-uno.component.html',
  styleUrl: './chat-uno.component.scss'
})
export class ChatUnoComponent implements OnInit, AfterViewChecked{
  ...
  constructor(public auth : AuthService, public chat : ChatService, private router: Router){ addIcons({arrowBackCircleOutline})}
  
  
  ngOnInit() {
    this.chat.obtenerChatUno().subscribe((respuesta) =>   {
  ...
  EnviarMensaje(){
    ...
    if(this.mensajeNuevo != "")
    {
      ...
      this.chat.agregarChatUno(this.mensaje);
